...
export class UpsertConnectProfileDto {
  ...
  @IsArray() @ArrayMaxSize(6) @IsString({ each: true }) @IsOptional()
  languages?: string[];
  ...
  @IsArray() @ArrayMaxSize(8) @IsString({ each: true }) @IsOptional()
  interests?: string[];
...
export class DiscoveryDto extends PageOptionsDto {
  ...
  @Type(() => Number) @IsInt() @Min(0) @Max(120) @IsOptional()
  minAge?: number;

  @ApiPropertyOptional() @Type(() => Number) @IsInt() @Min(18) @Max(120) @IsOptional()
  maxAge?: number;
  ...
  @ApiPropertyOptional({ type: [String], description: 'Matches if the profile speaks ANY of them.' })
  ...
  @IsIn(['RECOMMENDED', 'RECENT', 'NEAREST']) @IsOptional()
...
export class ListConnectionRequestsDto extends PageOptionsDto {
  ...
  @IsIn(['PENDING', 'ACCEPTED', 'DECLINED', 'CANCELLED', 'ALL']) @IsOptional()
