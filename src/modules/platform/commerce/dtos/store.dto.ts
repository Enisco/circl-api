...
export class OpeningHoursDto {
  ...
  @Type(() => Number) @IsInt() @Min(0) @Max(1439) @IsOptional()
...
export class CreateStoreDto {
  ...
  @ApiProperty({ minLength: 2, maxLength: 80, description: 'The area within the city, e.g. "Moss Side".' })
  ...
  @ApiPropertyOptional() @Trim() @IsString() @MaxLength(200) @IsOptional()
  addressLine1?: string;

  @ApiPropertyOptional() @Trim() @IsString() @MaxLength(20) @IsOptional()
  postcode?: string;
  ...
  @ApiPropertyOptional({ type: [String], description: "Max 4. Suggested from the seller's own heritage." })
  @IsArray() @ArrayMaxSize(4) @IsString({ each: true }) @IsOptional()
  heritageTags?: string[];

  @ApiPropertyOptional({ type: [String], description: 'Item category codes. Derivable from items, so a hint.' })
  @IsArray() @ArrayMaxSize(12) @IsString({ each: true }) @IsOptional()
  categories?: string[];

  @ApiPropertyOptional({ type: [StoreContactDto], description: 'PHONE prefilled from the profile.' })
  @IsArray() @ValidateNested({ each: true }) @Type(() => StoreContactDto) @IsOptional()
  contact?: StoreContactDto[];

  @ApiPropertyOptional({ type: [OpeningHoursDto], description: 'Exactly 7 entries when sent.' })
  @IsArray() @ArrayMinSize(7) @ArrayMaxSize(7) @ValidateNested({ each: true })
  @Type(() => OpeningHoursDto) @IsOptional()
...
export class UpdateStoreDto extends CreateStoreDto {
  ...
  @Trim() @IsString() @MinLength(2) @MaxLength(60) @IsOptional()
  declare name: string;
...
export class StoreStatusDto {
  @ApiProperty({
    enum: StoreStatus,
    description: 'Overrides opening hours. A store on holiday is not open even at 10am on a Tuesday.',
  })
  @IsEnum(StoreStatus)
  status: StoreStatus;
}
...
export class ItemDto {
  ...
  @Trim() @IsString() @MinLength(2) @MaxLength(100)
  name: string;

  @ApiProperty({ description: 'Pence, greater than zero.' })
  @Type(() => Number) @IsInt() @Min(1, { message: 'price must be greater than zero' })
  price: number;
  ...
  @ApiPropertyOptional({ description: 'For the genuine exceptions the code list does not cover (D21).' })
  @Trim() @IsString() @MaxLength(40) @IsOptional()
  ...
  @Trim() @IsString() @MaxLength(200) @IsOptional()
  options?: string;
...
export class UpdateItemDto extends ItemDto {
  ...
  @Trim() @IsString() @MinLength(2) @MaxLength(100) @IsOptional()
  declare name: string;
...
export class BrowseCommerceDto extends PageOptionsDto {
  ...
  @IsIn(['RECOMMENDED', 'NEAREST', 'RATING', 'NEWEST', 'PRICE_LOW', 'PRICE_HIGH']) @IsOptional()
...
export class ListStoreItemsDto extends PageOptionsDto {
  ...
  @ApiPropertyOptional({
    default: false,
    description: 'An out-of-stock item still tells a buyer what the shop sells, so this defaults off.',
  })
  @Bool() @IsBoolean() @IsOptional()
...
export class CreateEnquiryDto {
  ...
  @Trim() @IsString() @MinLength(6) @MaxLength(300) @IsOptional()
...
export class AiDraftItemsDto {
  ...
  @IsIn(['WARM', 'SHORT', 'FORMAL']) @IsOptional()
  tone?: 'WARM' | 'SHORT' | 'FORMAL';

  @ApiPropertyOptional({ description: "Lets the model match the store's existing categories and pricing." })
  @Trim() @IsString() @IsOptional()
  storeId?: string;
}
