-- DropIndex
DROP INDEX "community_offers_title_trgm_idx";

-- DropIndex
DROP INDEX "community_requests_description_trgm_idx";

-- DropIndex
DROP INDEX "community_requests_title_trgm_idx";

-- DropIndex
DROP INDEX "connect_profiles_looking_for_trgm_idx";

-- DropIndex
DROP INDEX "groups_name_trgm_idx";

-- DropIndex
DROP INDEX "guides_intro_trgm_idx";

-- DropIndex
DROP INDEX "guides_title_trgm_idx";

-- DropIndex
DROP INDEX "professional_listings_title_trgm_idx";

-- DropIndex
DROP INDEX "request_responses_one_offer_per_author_idx";

-- DropIndex
DROP INDEX "reviews_one_prior_work_per_pair_idx";

-- DropIndex
DROP INDEX "store_items_name_trgm_idx";

-- DropIndex
DROP INDEX "stores_name_trgm_idx";

-- DropIndex
DROP INDEX "user_profile_bio_trgm_idx";

-- DropIndex
DROP INDEX "user_profile_can_help_with_trgm_idx";

-- DropIndex
DROP INDEX "users_first_name_trgm_idx";

-- DropIndex
DROP INDEX "users_last_name_trgm_idx";

-- DropIndex
DROP INDEX "users_username_trgm_idx";
