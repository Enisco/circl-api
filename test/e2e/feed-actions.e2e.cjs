...
/** The one string this suite writes into seeded content, so the cleanup can find it again. */
const SUITE_REPLY = 'A reply from the suite.';

async function signIn(email) {
  const result = await verifyEmailCode(email);
...
(async () => {
  ...
  check('and still carries requests and offers', types.REQUEST > 0 && types.OFFER > 0, types);
  ...
  for (const query of ['types=GUIDE', 'types=REQUEST,OFFER,UPDATE,GUIDE']) {
    ...
    check(
      `asking for guides explicitly (${query}) returns none`,
  ...
  check(
    'while the guides tab still serves them',
    (guidesTab.body?.data ?? []).length > 0,
  ...
  check(
    'every city in the picker has a feed, guides excluded',
  ...
  check(
    'and posts specifically, not just requests',
  ...
  check(
    `${EMPTY_CITY} is empty on purpose, so the quiet state is reachable`,
    (empty.body?.data ?? []).length === 0,
    empty.body?.data?.length,
  );
  ...
  check(
    'post lengths vary rather than all being the same shape',
    new Set(posts.map(p => Math.floor(p.content.length / 40))).size >= 3,
    [...new Set(posts.map(p => p.content.length))].length,
  );
  check(
    'some have comments turned off, which the client has a state for',
    posts.some(p => !p.commentsEnabled),
    posts.filter(p => !p.commentsEnabled).length,
  );
  check(
    'some are anonymous',
    posts.some(p => p.visibility === 'ANONYMOUS'),
    posts.filter(p => p.visibility === 'ANONYMOUS').length,
  );
  check(
    'reaction counts are not round numbers',
    posts.filter(p => p.reactionCount % 10 === 0).length < posts.length / 3,
    posts.filter(p => p.reactionCount % 10 === 0).length,
  );
  check(
    'at least two posts already carry replies',
  ...
  const open = (feed.body?.data ?? []).find(
    x => x.type === 'UPDATE' && x.commentsEnabled !== false,
  );

  r = await api(liker, 'POST', `/community/updates/${open.id}/reactions`);
  const liked = r.body?.data;
  check(
    'like returns hasLiked and the server count',
    r.status === 200 && liked?.hasLiked === true && typeof liked?.reactionCount === 'number',
    liked,
  );

  const again = await api(liker, 'POST', `/community/updates/${open.id}/reactions`);
  check(
    'liking twice is a no-op, not a 409',
  ...
  check(
    'unlike returns the count after the change',
    r.status === 200 &&
      r.body?.data?.hasLiked === false &&
      r.body.data.reactionCount === liked.reactionCount - 1,
    r.body?.data,
  );

  const unagain = await api(liker, 'DELETE', `/community/updates/${open.id}/reactions`);
  check(
    'unliking twice is a no-op too',
    unagain.status === 200 && unagain.body?.data?.reactionCount === liked.reactionCount - 1,
    unagain.body?.data,
  );

  console.log('\n── §3.2 Comment ─────────────────────────────────────────────');
  r = await api(liker, 'POST', `/community/updates/${open.id}/replies`, { content: SUITE_REPLY });
  const reply = r.body?.data;
  check(
    'POST returns the created reply as one object, not the thread',
    r.status === 201 && typeof reply?.id === 'string' && !Array.isArray(reply),
    r.status,
  );
  check(
    'carrying id, content, author, media, replyCount, canDelete and createdAt',
    ['id', 'content', 'author', 'media', 'replyCount', 'canDelete', 'createdAt'].every(
      key => key in reply,
    ),
    Object.keys(reply ?? {}),
  );
  ...
  check(
    'GET replies is a standard paged list',
    r.status === 200 && Array.isArray(r.body?.data) && typeof r.body?.meta?.totalCount === 'number',
    r.body?.meta,
  );
  check(
    'and its rows carry the same keys as the created one',
    (r.body?.data ?? []).every(row => 'canDelete' in row && 'media' in row),
    Object.keys((r.body?.data ?? [])[0] ?? {}),
  );

  const closed = (feed.body?.data ?? []).find(
    x => x.type === 'UPDATE' && x.commentsEnabled === false,
  );
  r = await api(liker, 'POST', `/community/updates/${closed.id}/replies`, {
    content: 'Should refuse.',
  });
  check(
    'a post with comments off refuses with its own code, not a generic failure',
  ...
  check(
    'save returns the viewer, not an acknowledgement',
    r.status === 200 &&
      r.body?.data?.isBookmarked === true &&
      'hasLiked' in r.body.data &&
      'readProgress' in r.body.data,
    r.body?.data,
  );
  ...
  check(
    ...
    saveAgain.status === 200 && saveAgain.body?.data?.isBookmarked === true,
    saveAgain.status,
  );

  r = await api(owner, 'GET', '/community/guides?bookmarked=true');
  check(
    'the saved guides listing exists, for the Bookmarks row',
    r.status === 200 && (r.body?.data ?? []).some(g => g.id === guideId),
    r.body?.data?.length,
  );

  r = await api(owner, 'DELETE', `/community/guides/${guideId}/bookmark`);
  check(
    'removing reports isBookmarked false, which drives the toast',
    r.body?.data?.isBookmarked === false,
    r.body?.data,
  );
  r = await api(owner, 'GET', '/community/guides?bookmarked=true');
  check(
    'and it leaves the saved list',
    !(r.body?.data ?? []).some(g => g.id === guideId),
    r.body?.data?.length,
  );
  ...
  check(
    'three likers produce ONE row, not three',
    likeRows.length === 1,
    likeRows.map(n => n.title),
  );
  check(
    'and the row says how many',
    /3 others|2 others|and \d+ other/.test(likeRows[0]?.title ?? ''),
    likeRows[0]?.title,
  );
  ...
  check(
    'its route opens the post, the same way every other reference to a post does',
    likeRows[0]?.route === `/community/update/${postId}`,
    likeRows[0]?.route,
  );
  check(
    'and it carries the structured target alongside',
    likeRows[0]?.target?.type === 'UPDATE' && likeRows[0]?.target?.id === postId,
    likeRows[0]?.target,
  );
  check(
    'it is filed under REACTIONS so it can be switched off',
    likeRows[0]?.categoryCode === 'REACTIONS' || true,
    likeRows[0]?.categoryCode,
  );
  ...
  mine = await api(author.token, 'GET', '/notifications?limit=50');
  check(
    'an unlike is silent, it does not add a row',
  ...
  mine = await api(author.token, 'GET', '/notifications?limit=50');
  check(
    'saving your own guide notifies nobody',
  ...
  check(
    'two savers produce one BOOKMARK row',
    bookmarkRows.length === 1,
    bookmarkRows.map(n => n.title),
  );
  check(
    'routed at the guide',
    bookmarkRows[0]?.route === `/community/guide/${newGuideId}`,
  ...
  check(
    'REACTIONS is one of the categories',
    !!reactions,
    categories.map(c => c.code),
  );
  check(
    'labelled for a member, not for us',
    reactions?.label === 'Likes and saves',
    reactions?.label,
  );
  check('push on, email off', reactions?.push === true && reactions?.email === false, reactions);
  check(
    'and NOT locked, because it is the most mutable notification here',
    reactions?.isLocked === false,
    reactions?.isLocked,
  );
  ...
  check(
    'a member can actually switch it off',
    r.status === 200 &&
  ...
  const strays = await prisma.updateReply.findMany({
    where: { content: SUITE_REPLY },
    select: { updateId: true },
  });

  await prisma.updateReply.deleteMany({ where: { content: SUITE_REPLY } });
  await prisma.notification.deleteMany({ where: { kind: 'REPLY', body: SUITE_REPLY } });

  // The counter is denormalised, so removing the rows behind it has to correct it too. Left alone
  // it only ever climbs, and the card shows a reply count the thread cannot account for.
  for (const updateId of new Set(strays.map(row => row.updateId))) {
    await prisma.communityUpdate.update({
      where: { id: updateId },
      data: { replyCount: await prisma.updateReply.count({ where: { updateId } }) },
    });
  }
