...
const { api, check, finish, prisma, verifyEmailCode } = require('./harness.cjs');
...
async function signIn(email) {
  ...
  return { ok: result.ok, token: result.cookieToken };
}
...
(async () => {
  ...
  check(
    'it has a bio, a city and a country',
    (() => {
      const d = r.body?.data ?? {};
  ...
  check(
    'and a signed avatar url',
    typeof r.body?.data?.avatarUrl === 'string',
    r.body?.data?.avatarUrl,
  );
  check(
    'the avatar object actually exists',
    await objectExists(r.body?.data?.avatarUrl),
    r.body?.data?.avatarUrl,
  );
  ...
  check(
    'a city filter by NAME returns rows (1.0.3)',
    (r.body?.data ?? []).length > 0,
    r.body?.meta?.totalCount,
  );
  check(
    'times read as hours and days, not months',
    (() => {
  ...
  check(
    'and it names no author',
    anonymous.every(row => row.author?.id === null),
    anonymous[0]?.author,
  );

  const empty = await api(token, 'GET', '/community/requests?cityId=Nottingham');
  check(
    'a city with nothing in it returns an empty list, not an error',
    empty.status === 200,
    empty.status,
  );
  ...
  check(
    'every listing is UNVERIFIED (D13)',
  ...
  check(
    'at least one needs their action',
    (r.body?.data ?? []).some(row => row.needsYourAction),
    (r.body?.data ?? []).map(b => b.state),
  );
  ...
  check(
    'connect discovery returns people',
    (r.body?.data ?? []).length > 0,
    r.body?.meta?.totalCount,
  );
  ...
  check('all three buckets are represented', new Set(rows.map(n => n.bucket)).size === 3, [
    ...new Set(rows.map(n => n.bucket)),
  ]);
  check(
    'VERIFICATION pins to the top despite being the oldest',
    rows[0]?.kind === 'VERIFICATION',
    rows.slice(0, 2).map(n => `${n.kind}@${n.createdAt}`),
  );
  check(
    'a row with a null route exists',
    rows.some(n => n.route === null),
    rows.map(n => n.route),
  );
  ...
  check(
    'the preference matrix is the nine rows',
    (r.body?.data?.categories ?? []).length === 9,
    r.body?.data?.categories?.length,
  );
  check(
    'with COMPLIANCE locked',
  ...
  r = await api(token, 'GET', '/messages');
  check(
    'the support thread is pinned to the top of the inbox',
    (r.body?.data ?? [])[0]?.isPinned === true,
    (r.body?.data ?? [])[0],
  );
  ...
  check(
    'the store has a logo url',
    typeof detail.body?.data?.logoUrl === 'string',
    detail.body?.data?.logoUrl,
  );
  check(
    'and the logo object exists',
    await objectExists(detail.body?.data?.logoUrl),
    detail.body?.data?.logoUrl,
  );
  ...
  check(
    'an out-of-stock item is in the catalogue',
  ...
  const admin = await prisma.user.create({
    data: {
      firstName: 'E2E',
      lastName: 'pulse',
      email: `e2e-pulse-${stamp}@example.test`,
      username: `e2e_pulse_${stamp}`,
      status: 'ACTIVE',
      isStaff: true,
  ...
  check(
    'at least one Manchester dashboard shows numbers',
    Object.values(pulses).some(d => (d.bars ?? []).length > 0),
    Object.fromEntries(Object.entries(pulses).map(([k, v]) => [k, v.contributingMembers])),
  );
  check(
    'Connect clears its floor of 20 too, from the extra profiles',
    (pulses.connect.contributingMembers ?? 0) >= 20,
    pulses.connect.contributingMembers,
  );
  check(
    'no dashboard names anybody',
    !JSON.stringify(pulses).includes('avatarUrl') &&
      !JSON.stringify(pulses).includes('displayName'),
    'a person appeared in a Pulse payload',
  );

  const thin = await api(token, 'GET', '/pulse/community?cityId=Leeds');
  check(
    'a thin city suppresses, as B.3 intends',
    (thin.body?.data?.bars ?? []).length === 0,
    thin.body?.data,
  );
  ...
  check(
    'member 2 has no avatar, so the initials fallback is visible',
    r.body?.data?.avatarUrl === null,
    r.body?.data?.avatarUrl,
  );
  ...
  check(
    'the one-word name renders without a trailing null',
    r.body?.data?.user?.displayName === 'Aiyana',
    r.body?.data?.user?.displayName,
  );
  ...
  check(
    'the groups tab has groups',
    groups.length >= 2,
    groups.map(g => g.name),
  );
  check(
    'with member counts, not zeroes',
    groups.every(g => g.memberCount > 0),
    groups.map(g => g.memberCount),
  );
  check(
    'member 1 is in one of them',
    groups.some(
      g =>
        g.viewer?.membership === 'MEMBER' ||
        g.viewer?.membership === 'ADMIN' ||
        g.viewer?.state === 'MEMBER' ||
        g.viewer?.state === 'ADMIN',
    ),
  ...
  check(
    'and a post carries its replies',
    posts.some(post => post.counts?.replies > 0),
  ...
  check(
    'a store carries its opening hours',
    (storefront.openingHours ?? []).some(h => h.openMinutes !== null),
    storefront.openingHours,
  );
  check(
    'so "Open now" is computed rather than always false',
    typeof storefront.isOpenNow === 'boolean',
    storefront.isOpenNow,
  );
  check(
    'and its heritage tags render as chips',
    (storefront.heritageTags ?? []).length > 0,
  ...
  check(
    'a booking has a timeline with stages actually reached',
  ...
  check(
    'a booking slot is still in the future, not months past (B.2.1)',
  ...
  check(
    'with COMPLIANCE locked on',
    categories.some(c => c.code === 'COMPLIANCE' && c.isLocked),
