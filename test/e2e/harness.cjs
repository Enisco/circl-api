...
async function sweep(label = 'sweep') {
  ...
  // Deleting the author does not delete the guide: `guides.authorId` is SetNull, because a
  // Circl-drafted guide legitimately has no author, so a guide OUTLIVES the member who wrote it.
  // Right in production, where deletion anonymises and the row survives; here it left every
  // boundary-test guide titled "aaaaaa" sitting in the Guides tab forever.
