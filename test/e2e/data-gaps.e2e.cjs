...
(async () => {
  ...
  check(
    'sign-in returns accessToken in the JSON body',
    typeof one.body?.accessToken === 'string',
  ...
  check(
    'the limits block carries all four numbers',
    tax.limits?.maxInterests === 8 &&
      tax.limits?.maxLanguages === 6 &&
      tax.limits?.minConnectAge === 18 &&
      tax.limits?.nearMeRadiusMiles === 5,
    tax.limits,
  );

  check(
    'connectAgeBands carry bounds, not a label to parse back',
    (tax.connectAgeBands ?? []).every(b => typeof b.minAge === 'number' && 'maxAge' in b),
    tax.connectAgeBands,
  );
  check(
    'and the open-ended band has a null maxAge',
    (tax.connectAgeBands ?? []).some(b => b.maxAge === null),
    tax.connectAgeBands,
  );

  check(
    'spokenLanguages carry an ISO code alongside the stable one',
    (tax.spokenLanguages ?? []).every(l => typeof l.iso === 'string'),
    (tax.spokenLanguages ?? []).slice(0, 3),
  );

  check(
    'privateHelpCategories keep the codes 6.3.1 fixed',
    ['HOUSING', 'IMMIGRATION', 'SAFETY', 'MONEY', 'HEALTH', 'WORK', 'OTHER'].every(code =>
      (tax.privateHelpCategories ?? []).some(t => t.code === code),
    ),
    (tax.privateHelpCategories ?? []).map(t => t.code),
  );
  check(
    'and OTHER reads as "Something else"',
    (tax.privateHelpCategories ?? []).find(t => t.code === 'OTHER')?.label === 'Something else',
  );

  check(
    'inactive terms are served too, so an old post still renders its name',
  ...
  check(
    'exactly one is the current device',
    sessions.filter(s => s.isCurrent).length === 1,
    sessions.map(s => s.isCurrent),
  );
  check(
    'each carries a device label and platform',
    sessions.every(s => s.device && ['IOS', 'ANDROID', 'WEB'].includes(s.platform)),
    sessions.map(s => ({ d: s.device, p: s.platform })),
  );
  check(
    'one is stale by weeks, which is why the screen exists',
  ...
  check(
    'the current session cannot be revoked from here',
  ...
  check(
    'and sign out everywhere reports what it actually revoked',
    r.status === 200 && r.body?.data?.revoked === 1,
    r.body?.data,
  );
  r = await api(revoker.token, 'GET', '/users/me/sessions');
  ...
  check(
    'all four checks are returned',
    checks.length === 4,
    checks.map(c => c.check),
  );
  check(
    'each carries updatedAt and note',
    checks.every(c => 'updatedAt' in c && 'note' in c),
    checks[0],
  );
  check('EMAIL is verified', checks.find(c => c.check === 'EMAIL')?.status === 'VERIFIED');
  check(
    'D13: nothing else is VERIFIED, ever',
  ...
  check(
    'one member sits IN_REVIEW so the state is demonstrable',
  ...
  check(
    'a different professional has a different week, not the same grid',
  ...
  check(
    'the profile returns interests',
    (r.body?.data?.profile?.interests ?? []).length >= 4,
  ...
  check(
    'privacy loads with all three switches',
    typeof r.body?.data?.personalisedFeed === 'boolean' &&
      typeof r.body?.data?.useActivityForRecommendations === 'boolean' &&
      typeof r.body?.data?.showInConnectDiscovery === 'boolean',
    r.body?.data,
  );

  r = await api(token, 'PATCH', '/users/me/privacy', { personalisedFeed: false });
  check(
    'a subset patches and the whole object comes back',
    r.status === 200 &&
      r.body?.data?.personalisedFeed === false &&
      'showInConnectDiscovery' in (r.body?.data ?? {}),
    r.body?.data,
  );

  r = await api(token, 'GET', '/community/feed');
  check(
    'and the feed really does fall back to recency',
    r.body?.meta?.ranking === 'LATEST',
    r.body?.meta,
  );
  ...
  r = await api(token, 'GET', '/community/feed');
  check(
    'switching it back restores ranking',
    r.body?.meta?.ranking === 'PERSONALISED',
    r.body?.meta,
  );
  ...
  check(
    'the Commerce scope is actually searched, not always empty',
    (r.body?.data?.groups ?? []).length > 0,
    (r.body?.data?.groups ?? []).map(g => g.type),
  );
  check(
    'and returns only Commerce types',
  ...
  check(
    'a one-character term is empty groups, not a 400',
    r.status === 200 && (r.body?.data?.groups ?? []).length === 0,
    r.status,
  );

  r = await api(token, 'GET', '/search?q=whiting');
  check(
    'suggestions come back with the groups',
    Array.isArray(r.body?.data?.suggestions),
  ...
  r = await api(mine.token, 'POST', '/users/me/data-export');
  check(
    'a second while one is pending is refused, not queued',
  ...
  check(
    'an address on another account is refused',
  ...
  check(
    'a pending request is recorded against the NEW address',
    pending?.newEmail === 'moved@example.test',
    pending?.newEmail,
  );
  ...
  r = await api(mine.token, 'POST', '/users/me/email/change/confirm', { code: '000000' });
  check(
    'an expired one is 410 CODE_EXPIRED',
  ...
  check(
    'a store that shows its address has real coordinates',
    typeof r.body?.data?.address?.latitude === 'number' &&
      typeof r.body?.data?.address?.longitude === 'number',
    r.body?.data?.address,
  );
  check('and is not marked approximate', r.body?.data?.address?.isApproximate === false);

  const hidden = await prisma.store.findFirst({
    where: { hidesExactAddress: true },
    select: { id: true },
  });
  r = await api(token, 'GET', `/commerce/stores/${hidden.id}`);
  check(
    'one that hides it sends no line1 or postcode',
    r.body?.data?.address?.line1 === null && r.body?.data?.address?.postcode === null,
    r.body?.data?.address,
  );
  check(
    'and its pin is rounded to about a kilometre, flagged approximate',
  ...
  check(
    'a store that shows its address carries a staticMapUrl',
    typeof mapUrl === 'string' && mapUrl.startsWith('http'),
    mapUrl,
  );
  check(
    'it is a signed URL, not a provider URL with a key in it',
  ...
  check(
    'and is a real PNG of the size the doc asks for',
    bytes.subarray(0, 8).toString('hex') === '89504e470d0a1a0a' &&
      bytes.readUInt32BE(16) === 640 &&
      bytes.readUInt32BE(20) === 220,
    { w: bytes.readUInt32BE(16), h: bytes.readUInt32BE(20) },
  );

  r = await api(token, 'GET', `/commerce/stores/${hidden.id}`);
  check(
    'a store that hides its address gets no tile at all',
    r.body?.data?.staticMapUrl === null,
    r.body?.data?.staticMapUrl,
  );
  ...
  check(
    'a store created through the API gets its tile built on first read',
    typeof backfilled === 'string',
    backfilled,
  );

  const backfilledTile = backfilled ? await fetch(backfilled) : null;
  check(
    'and that object is a real PNG too',
    backfilledTile?.status === 200 &&
      Buffer.from(await backfilledTile.arrayBuffer())
        .subarray(0, 8)
  ...
  check(
    'the five UK lines are all there',
    [
      'Samaritans',
  ...
  check(
    'each carries a number and a description the app can render',
    resources.every(x => x.number && x.description),
    resources[0],
  );
  check(
    'crisis lines sort first',
    resources[0]?.isCrisis === true,
    resources.map(x => x.isCrisis),
  );
  check(
    'lastChecked is current, not a date baked into a binary',
    !!r.body?.meta?.lastChecked && Date.now() - new Date(r.body.meta.lastChecked) < 40 * 86400000,
    r.body?.meta,
  );
  ...
  check(
    'every seeded thread holds six or more messages',
    threads.length > 0 && threads.every(t => t.messageCount >= 6),
    threads.map(t => t.messageCount),
  );
  check(
    'spread over days rather than minutes',
    threads.every(t => {
  ...
  r = await api(
    seller.body.accessToken,
    'GET',
    `/commerce/stores/${myStore.body?.data?.id}/demand-hints`,
  );
  check('the demand card has something to show', (r.body?.data ?? []).length >= 2, r.body?.data);
  check(
    'each hint carries a term, a count and a city',
    (r.body?.data ?? []).every(h => h.term && h.searches >= 3 && h.cityName),
    r.body?.data?.[0],
  );
  ...
  const post = await api(victim.token, 'POST', '/community/requests', {
    categoryCode: 'JOBS',
    title: 'A seeded request to report',
  ...
  check('the sheet payload is accepted as sent', r.status === 202, {
    s: r.status,
    b: r.body?.error,
  });
  ...
  check(
    'blockUserId applied the block in the same call',
    (r.body?.data ?? []).some(b => b.user?.id === victim.id),
    r.body?.data,
  );
  ...
  r = await api(mine.token, 'POST', '/users/notification-preferences/device-token', {
    token: 'seed-fcm-1',
  });
  check('a token registers', r.status === 200, r.status);

  const second = await makeUser('gapsphone');
  await api(second.token, 'POST', '/users/notification-preferences/device-token', {
    token: 'seed-fcm-1',
  });
  const holders = await prisma.pushDevice.count({ where: { token: 'seed-fcm-1' } });
  check(
    'the same handset on a second account MOVES rather than sitting on both',
    holders === 1,
    holders,
  );

  r = await api(second.token, 'DELETE', '/users/notification-preferences/device-token', {
    token: 'seed-fcm-1',
  });
  ...
  await prisma.user.deleteMany({
    where: { id: { in: [mine.id, second.id, victim.id, revoker.id, mapper.id] } },
  });
  await sweep('cleanup');
  await finish();
})().catch(fail);
